...
import { useCreateVisit, useUpdateVisit, useVisitFull } from "../hooks/use-visits";
import { nowLocalInput, toLocalInput, type VisitForm as RHF } from "./visit-form-helpers";
...
const STEP_META: Record<string, { title: string; description: string }> = {
  patient: { title: "Patient & visit", description: "Select the patient and the reason for this visit." },
  vitals: { title: "Vitals", description: "Growth measurements and vital signs." },
  exam: { title: "Examination", description: "History of present illness and physical exam findings." },
  screening: { title: "Screening & notes", description: "Vision, hearing, dental and developmental screening." },
  medications: { title: "Medications", description: "Medications prescribed at this visit." },
  vaccination: { title: "Vaccination", description: "Vaccines administered at this visit." },
  diagnosis: { title: "Diagnosis", description: "Diagnoses (ICD-10) for this encounter." },
  attachments: { title: "Attachments", description: "Files linked to this visit." },
  summary: { title: "Summary & payment", description: "Plan, fee and visit summary." },
};
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-6 md:px-10">
      ...
      >
        <StepperNav className="gap-1 overflow-x-auto pb-1">
          {STEPS.map((s, i) => (
            <StepperItem key={s.id} step={i + 1} className="flex flex-col items-center gap-1">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button type="button" variant="outline" onClick={goBack} disabled={idx === 0}>
            Back
          </Button>
          {currentId && (
            <Button
          ...
          {isLast ? (
            <Button type="button" onClick={complete} disabled={isSaving || !patientId}>
              {isSaving ? (
                <Loader2 className="size-4 animate-spin" />
              ) : (
            ...
            <Button type="button" onClick={goNext} disabled={isSaving || (idx === 0 && !patientId)}>
              {isSaving && idx === 0 ? (
                <Loader2 className="size-4 animate-spin" />
              ) : null}
              Next
